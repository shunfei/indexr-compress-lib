...
#ifndef __COMPRESSION_DATASTREAM_H
#define __COMPRESSION_DATASTREAM_H

#include <memory.h>

#include "defs.h"
#include "dep/bhassert.h"
...
protected:
	...
	uint len,	// length of 'buf', in bits
		 pos;   // number of bits of buf already read/wrote, counting from the beg. of buf (0 <= pos < len)
...
public:
	...
	void Reset()										{ pos = 0; }
	void Reset(uint len_, uint pos_ = 0)				{ len = len_; pos = pos_; }
	void Reset(char* buf_, uint len_, uint pos_ = 0)	{ buf = (uchar*)buf_; len = len_; pos = pos_; }
...
// During writing, some more bits than is needed can be cleared in advance - due to efficiency reasons.
class BitStream : public DataStream
{
	uint clrlen;	// index of the last cleared bit of 'buf' plus 1 (0 - no bit cleared yet)

	// clear bits of 'buf', from bit no. 'beg' to 'end'-1;
	// there is no checking of buffer overrun
	void ZeroBits(uint beg, uint end);
	// clear some number of bits in advance, beggining from 'pos', to allow faster Put operations
	void ClearBits();

public:
	// len_, pos_ - numbers of BITS
	void Reset()										{ DataStream::Reset(); clrlen = 0; }
	void Reset(uint len_, uint pos_ = 0)				{ DataStream::Reset(len_,pos_); clrlen = 0; }
	void Reset(char* buf_, uint len_, uint pos_ = 0)	{ DataStream::Reset(buf_,len_,pos_); clrlen = 0; }
	...
	// these two methods are NOT virtual, so can be used directly to slightly increase efficiency
	...
	void PutUInt64(_uint64 n, unsigned short int no_bits);
	_uint64 GetUInt64(unsigned short int no_bits);

	void FastPutBlock(char* data, uint n);		// append 'n' bytes of 'data', starting at the byte boundary in 'buf'
	void FastGetBlock(char* data, uint n);		// read 'n' bytes to 'data'

	virtual uchar Get()			{ return GetBit(); }
	virtual void Put(uchar v)	{ PutBit(v);       }
};
...
{
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT(no_bits <= 32);
...
inline uint BitStream::GetUInt(unsigned short int no_bits)
{
	...
	for(; no_bits - i >= 8; i += 8)
		result |= (((uint)GetByte()) << i);
	for(; i < no_bits; i++)
		result |= (((uint)GetBit()) << i);
	return result;
}

inline void BitStream::PutUInt64(_uint64 n, unsigned short int no_bits)
{
	BHASSERT_WITH_NO_PERFORMANCE_IMPACT(no_bits <= 64);
	int i = 0;		// no. of bits already stored
	for(; no_bits - i >= 8; i += 8)
		PutByte((uchar)(n >> i));
	for(; i < no_bits; i++)
...
inline unsigned long long BitStream::GetUInt64(unsigned short int no_bits)
{
	...
	int i = 0;		// no. of bits already read
	for(; no_bits - i >= 8; i += 8)
		result |= (((_uint64)GetByte()) << i);
	for(; i < no_bits; i++)
		result |= (((_uint64)GetBit()) << i);
	return result;
}
